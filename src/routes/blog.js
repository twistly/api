...
router.get('/:blogUrl/stats', resolveBlogUrl, async (req, res) => {
    ...
    const stats = await TumblrStat.find({blogId}, '-_id -__v -blogId').sort('date').limit(40).exec();
    res.send({stats});
});
...
router.get('/', async (req, res, next) => {
    ...
    const blogs = flatten(user.tumblr.map(tumblr => tumblr.blogs));
    if (blogs.length >= 1) {
...
router.get('/:blogUrl/followers', resolveBlogUrl, (req, res) => {
    const blog = req.blog;
